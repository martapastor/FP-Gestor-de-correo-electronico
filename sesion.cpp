...
void lecturaRapida (tSesion &sesion) {
	...
	for (int i = 0; i < bandejaActiva->cont; i++) {
		if (!bandejaActiva->correo[i].leido){		
			if (buscar(sesion.gestor->listCorreos, bandejaActiva->correo[i].identificador, pos)) {
				correos.arrCorreos[correos.cont] =  &listaCorreos.correo[pos];
				correos.cont++;
			}
	...
	for (int i = 0; i < correos.cont; i++) {
		cout << to_string(*correos.arrCorreos[i]);
		cout << left << setw(80) << setfill('-') << '-';
	}
}
...
void ordenarSeleccion(tPuntCorreos &correos, int longitud) {
	for (int i = 0; i < longitud - 1; i++) {
		...
		for (int j = i + 1; j < longitud; j++) {
			if (ignorarREyFW(*correos.arrCorreos[j]) < ignorarREyFW(*correos.arrCorreos[menor])) menor = j;
		}

		//Intercambiamos el menor con correos.arrCorreos[i]
		if (i < menor) {
			tCorreo aux = *correos.arrCorreos[i];
			*correos.arrCorreos[i] = *correos.arrCorreos[menor];
			*correos.arrCorreos[menor] = aux;
		}
	}
}
